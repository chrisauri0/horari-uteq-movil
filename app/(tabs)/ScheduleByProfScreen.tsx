...
import { ActivityIndicator, Alert, Modal, Platform, ScrollView, StyleSheet, View } from 'react-native';
...
async function createGoogleEvent(prof: string, day: string, hour: string) {
  try {
    ...
    const profData = profesores.find(
      (p) => `${p.nombre} ${p.apellidos}`.trim().toLowerCase() === prof.trim().toLowerCase()
    );
...
const end = new Date(start);
    ...
    Alert.alert("Asesoría agendada", "Se agregó correctamente a tu Google Calendar 📅");
...
// vista de carga
  ...
  const freeHours = Array.from(new Set(freeSlots.filter(s => s.day === selectedDay || !selectedDay).map((s) => s.hour)));
              ...
              <Card key={prof} variant="elevated" style={styles.professorCard}>
                <View style={styles.cardHeader}>
                  <View style={styles.avatarPlaceholder}>
                    <ThemedText style={styles.avatarText}>{prof.charAt(0)}</ThemedText>
                  </View>
        ...
        {filteredProfs.length === 0 && (
          <Card variant="outlined" style={styles.emptyCard}>
            <ThemedText style={styles.emptyText}>
              {searchQuery ? "No se encontraron profesores" : "No hay profesores disponibles"}
            </ThemedText>
          </Card>
        )}
      </ScrollView>
      ...
      <Modal visible={modalVisible} transparent={true} animationType="fade">
        <View style={styles.modalContainer}>
          ...
          <Card variant="elevated" style={styles.modalContent}>
            ...
            <ThemedText style={styles.modalProfName}>
              {selectedProf}
            </ThemedText>
            ...
            <View style={styles.pickerContainer}>
              ...
              <View style={styles.pickerWrapper}>
                <Picker
                  ...
                  <Picker.Item label="Seleccione un día" value="" color={UTEQColors.textSecondary} />
                  {freeDays.map((day) => (
                    <Picker.Item key={day} label={day} value={day} color={UTEQColors.textPrimary} />
                  ))}
                </Picker>
              </View>
            </View>
            ...
            <View style={styles.pickerContainer}>
              ...
              <View style={styles.pickerWrapper}>
                <Picker
                  ...
                  dropdownIconColor={UTEQColors.bluePrimary}>
                  <Picker.Item label="Seleccione una hora" value="" color={UTEQColors.textSecondary} />
                  {freeHours.map((hour) => (
                    <Picker.Item key={hour} label={`${hour}:00`} value={hour.toString()} color={UTEQColors.textPrimary} />
                  ))}
                </Picker>
              </View>
            </View>
